...
namespace stat2018
{
    public partial class oopuN : System.Web.UI.Page
    {
        ...
        private const string tenPlik = "oopu.aspx";
        private const string tenPlikNazwa = "oopu";
        private string path = "";
        ...
        protected void tabela_1()
        {
            ...
            DataTable tabelka01 = DevExpressXXL.zLicznikiemKolumn(dr.konwertujNaPrzecinek(dr.generuj_dane_do_tabeli_sedziowskiej_2019(int.Parse(idDzialu), 5, Date1.Date, Date2.Date, 111, tenPlik)));
            ...
            ASPxGridView1.Columns.Add(DevExpressXXL.podKolumna(teksty04, 63, idDzialu, false, szerokoscKolumny, "Liczba odroczonych publikacji wyroków/postanowień (Dz. 1.1.1. k.17)"));
            ASPxGridView1.Columns.Add(DevExpressXXL.podKolumna(teksty01, 66, idDzialu, false, szerokoscKolumny, "POZOSTAŁOŚĆ na następny m-c"));
            ASPxGridView1.Columns.Add(DevExpressXXL.podKolumna(new string[] { "Ogółem", "do 3 m-cy", "pow. 3 do 6 m - cy", "pow. 6 do 12 m - cy", "pow. 12 m-cy do 2 lat", "pow. 2 do 3 lat", "pow. 3 do 5 lat", "pow. 5 do 8 lat", "pow. 8 lat" }, 74, idDzialu, false, szerokoscKolumny, "pozostało spraw starych (wszystkie kategorie spraw) "));

            GridViewBandColumn liczbaSporzadzonychUzasadnien = DevExpressXXL.podKolumna(new string[] { "Łącznie", "w terminie ustawowym 14 dni", "razem po terminie ustawowym", "nie- usprawied- liwione" }, 75, idDzialu, false, szerokoscKolumny, "terminowość sporządzonych uzasadnień (wszystkie kategorie spraw)");

            GridViewBandColumn PoUplywie = (DevExpressXXL.podKolumna(new string[] { "1-14 dni", "w tym nieuspra -wiedliwione", "15-30 dni", "w tym nieuspra -wiedliwione", "powyżej 1 do 3 mies", "w tym nieuspra -wiedliwione", "ponad 3 mies", "w tym nieuspra -wiedliwione" }, 83, idDzialu, false, szerokoscKolumny, "po upływie terminiu ustawowego"));
            liczbaSporzadzonychUzasadnien.Columns.Add(PoUplywie);
            liczbaSporzadzonychUzasadnien.Columns.Add(DevExpressXXL.kolumnaDoTabeli("Uzasadnienia wygłoszone", "d_95", idDzialu, "", false, szerokoscKolumny));
            liczbaSporzadzonychUzasadnien.Columns.Add(DevExpressXXL.kolumnaDoTabeli("Liczba spraw do których wpłynął wniosek o transkrypcje uzasadnień wygłoszonych", "d_96", idDzialu, "", false, szerokoscKolumny));

            GridViewBandColumn liczbaSpraw = DevExpressXXL.podKolumna(new string[] { "razem", "w tym, w których projekt został zaakceptowany przez sędziego" }, 97, idDzialu, false, szerokoscKolumny, "Liczba spraw, w których projekt uzasadnienia orzeczenia sporządził asystent**");
            liczbaSporzadzonychUzasadnien.Columns.Add(liczbaSpraw);
            ASPxGridView1.Columns.Add(liczbaSporzadzonychUzasadnien);
            ASPxGridView1.Columns.Add(DevExpressXXL.SkargiNaPrzewleklosc(99, idDzialu, szerokoscKolumny));
            ...
            mediacjeWplyw.Columns.Add(DevExpressXXL.kolumnaDoTabeli("liczba spraw w których strony skierowano do mediacji", "d_103", idDzialu, "", false, szerokoscKolumny));
            mediacje.Columns.Add(mediacjeWplyw);

            mediacje.Columns.Add(DevExpressXXL.podKolumna(new string[] { "liczba ugód zawartych przed mediatorem", "w tym zatwierdzono ugodę" }, 104, idDzialu, false, szerokoscKolumny, "ROZSTRZYGNIĘCIE"));
            ASPxGridView1.Columns.Add(mediacje);

            ASPxGridView1.Columns.Add(DevExpressXXL.podKolumna(new string[] { "Ogółem", "zakreś- lonych", "nie zakreś- lonych" }, 106, idDzialu, false, szerokoscKolumny, "stan spraw zawieszonych (wszystkie kategorie spraw)"));
            ASPxGridView1.Columns.Add(DevExpressXXL.kolumnaDoTabeli("UWAGI", "d_109", idDzialu, "", false, szerokoscKolumny));

            ASPxGridView1.Columns.Add(DevExpressXXL.podKolumna(new string[] { "na rozprawe", "na posie- dzenie" }, 110, idDzialu, false, szerokoscKolumny, "Kolumna kontrolna (wyznaczenia>=załatwień)"));
