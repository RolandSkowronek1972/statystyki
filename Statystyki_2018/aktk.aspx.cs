...
namespace stat2018
{
    public partial class aktk : System.Web.UI.Page
    {
        ...
        public static string tenPlik = "aktk.aspx";
        ...
        protected void odswiez()
        {
            ...
            try
            {
                DataTable Tabela2 = dr.generuj_dane_do_tabeli_wierszy2018(Date1.Date, Date2.Date, id_dzialu, 3, 4, 5, tenPlik);
                Session["tabelka003"] = Tabela2;
                pisz("tab_3_", 4, 5, Tabela2);
            }
            catch (Exception ex)
            {
                cm.log.Error(tenPlik + " " + ex.Message);
            ...
            try
            {
                DataTable Tabela4 = dr.generuj_dane_do_tabeli_wierszy2018(Date1.Date, Date2.Date, id_dzialu, 4, 8, 7, tenPlik);
                Session["tabelka004"] = Tabela4;
                pisz("tab_4_", 7, 6, Tabela4);
            }
            catch (Exception ex)
            {
                cm.log.Error(tenPlik + " " + ex.Message);
        ...
        {
            string path = Server.MapPath("Template") + "\\aktk.xlsx";
            FileInfo existingFile = new FileInfo(path);

            string download = Server.MapPath("Template") + @"\aktk";
            ...
            using (ExcelPackage MyExcel = new ExcelPackage(existingFile))
            {
                ...
                tabela.tworzArkuszwExcle(MyExcel.Workbook.Worksheets[1], table, 39, 0, 6, true, false, false, false, false);

                // druga
                DataTable table2 = (DataTable)Session["tabelka002"];
                tabela.tworzArkuszwExcle(MyExcel.Workbook.Worksheets[2], table2, 15, 0, 4, true, false, false, false, false);
        ...
        protected void naglowekTabeli_gwTabela1(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            ...
            {
                string path = Server.MapPath("XMLHeaders") + "\\aktk1.xml";
        ...
        protected void naglowekTabeli_gwTabela2(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                string path = Server.MapPath("XMLHeaders") + "\\aktk2.xml";
