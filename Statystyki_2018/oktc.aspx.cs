    ...
    public partial class oktc : System.Web.UI.Page
    {
        ...
        protected void makeLabels()
        {
            try
            {
                ...
                {
                    // cały miesiąc
                    tabela1Label.Text = "Informacja o ruchu spraw w Wydziale Cywilnym za miesiąc " + strMonthName + " " + DateTime.Parse(Date2.Text).Year.ToString() + " roku.";
                    tabela3Label.Text = "Informacje o ruchach spraw w Wydziale Cywilnym Sekcji Nieprocesowej za miesiąc " + strMonthName + " " + DateTime.Parse(Date2.Text).Year.ToString() + " roku.";
                }
                else
                {
        ...
        protected void komorkaExcela(ExcelWorksheet Arkusz, int wiersz, int kolumna, string wartosc, bool merge, int pocztekMerga, int koniecMerga)
        {

            if (merge)
            {
                Arkusz.Cells[pocztekMerga, koniecMerga].Merge = true;
           

            }
            Arkusz.Cells[wiersz, kolumna].Value = wartosc;
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            using (ExcelPackage MyExcel = new ExcelPackage(existingFile))
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    for (int j = 0; j < 10; j++)
                    {
                        komorkaExcela(MyWorksheet1, i + rowik+7, j  +3, tabelka002.Rows[i][j].ToString(), false, 1, 1);
                    }
                }
